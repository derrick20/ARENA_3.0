...
REMOTE = True
if REMOTE:
    CONFIG.set_default_api_key("9f1952be-a6bd-4c03-9113-2f479555e3c5")
...
def calculate_h(
    ...
    with model.trace(dataset.prompts, remote=REMOTE):
        # Model has:
        #   Transformer has:
            # h blocks - which have ln1, attn, ln2, mlp
            # A final ln - which associates with the unembed
        #   LM_Head

        # .output is available for every module basically?
        h: Float[Tensor, "b seq d_m"] = model.transformer.h[layer].output[0]
        # We want the last position
        last_token_resid = h[:, -1, :]
        logits = model.lm_head.output[:, -1, :] # This is the (b, d_vocab) logits. we take the max
        predicted_tokens = logits.argmax(-1) # Take the max position for each batch
        
        h = last_token_resid.mean(dim=0)

        h.save()
        predicted_tokens.save()


    completions = model.tokenizer.batch_decode(predicted_tokens) # Convert each token into a str
...
def intervene_with_h(
    ...
    Returns:
        ...
        completions_intervention: list of string completions for the zero-shot prompts, with
            intervention using the h-vector
    """
    # Gesture: we need to run it on the full prompt
    # Then, run twice on the other
    h = h.to(model.device)  # ← Add this line!
    print(model.device)
    with model.trace(remote=remote) as tracer:
        with tracer.invoke(zero_shot_dataset.prompts):
            completions_zero_shot = model.lm_head.output[:, -1, :].argmax(-1).save()
            
        with tracer.invoke(zero_shot_dataset.prompts):
            orig_resid = model.transformer.h[layer].output[0] # b, seq, d_m
            orig_resid[-1] += h # hmm, how do we access it directly though?
            completions_intervention = model.lm_head.output[:, -1, :].argmax(-1).save()

    return [model.tokenizer.batch_decode(completion) for completion in [completions_zero_shot, completions_intervention]]


# SKIP THIS ONE BECAUSE IT DOESN"T LIKE US TO USE THE TEST on whitelist NNSIGHT
# if MAIN:
...
correct_tokens = zero_shot_dataset.completions
# correct_ids = model.tokenizer(correct_tokens, return_tensors="pt", padding=True)["input_ids"] # We want 1D shape
...
def calculate_h_and_intervene_logprobs(
    ...
    """
    # So this is the exact same damn thing but we just put in the pprobs?

    with model.trace(remote=REMOTE) as tracer:
        with tracer.invoke(dataset.prompts):
            h = model.transformer.h[layer].output[0][:, -1, :].save()
            # This can be used throughout these other invocations, it's in scope?

        correct_tokens = zero_shot_dataset.completions
        correct_ids = [tokens[0] for tokens in model.tokenizer(correct_tokens)["input_ids"]] # We want 1D shape
        batch = len(correct_ids)
        
        with tracer.invoke(zero_shot_dataset.prompts):
            logits = model.lm_head.output[:, -1, :] # batch, last token.
            logprobs = logits.log_softmax(dim=-1)
            correct_logprobs = logprobs[range(batch), correct_ids].save() # we want to select those elements from the dataset's answers though
        ...
        with tracer.invoke(zero_shot_dataset.prompts):
            ...
            correct_logprobs_intervention = logprobs[range(batch), correct_ids].save() # we want to select those elements from the dataset's answers though
    # Okay, for the difficult, the new part
    # We just take the logits, do the softmax? logexpsum. The do exp
    # e^x / (sum e^y). x - logsumexp(x)
...
def calculate_fn_vectors_and_intervene(
    ...
    # Okay the sketch is 
    f'''
We want to have the clean run.

Then, for all selections of 0 or 1 heads from all layers, we will swap in the
h value taken from the clean run on the clean dataset.

Namely, we place in the z output of that head, and then multiply by the Wo.

We have access to the weights here, so we don't need to do any sort of ablating input trick

But yeah, we take that z[head] * Wo[head] and ADD that into the atten output

Let's like our blowjob - we'll skirt each head hierarchically. With medium depth.
    '''

    if not layers:
        layers = list(range(N_LAYERS))
    causal_indirect_effect = t.zeros(len(layers), N_HEADS)

    # an annoying 0 selection we have to
    exp_completions = [token_list[0] for token_list in model.tokenizer(dataset.completions).input_ids]
    batch = len(exp_completions)

    corrupt_prompts = dataset.create_corrupted_dataset().prompts
    orig_correct_logprobs = None
    with model.trace(remote=REMOTE) as tracer:
        # Run on clean
        clean_values = {}
        with tracer.invoke(dataset.prompts):
            for layer in layers:
                z = model.transformer.h[layer].attn.out_proj.input # The output of the attn then goes into the projection?
                # We just take the last position
                # We need to get just the values when we take the specific head through
                z_end = z[:, -1, :]
                z_heads = einops.rearrange(z_end, "batch (h d_head) -> batch h d_head", h=N_HEADS) # How do I get this syntax right?
                clean_values[layer] = z_heads

        # Patch in the clean spot
        # We'll repeat the above logic, just swapping in the exact z_end values.
        # So, we just select the output[0] there, and assign to our clean values across the batch, at layer, head
        # Do one initial run:
        print("Finished clean run")

        def collect_sperm():
                            # Then, we pull out the logprobs for the rest of therun
            logits = model.lm_head.output # Hmm, yeah so we have the final output is the logits? 
            # Shape (b, seq, d_vocab)
            logits = logits[:, -1]
            # i thought we had to do something different earlier?
            logprobs = logits.log_softmax(dim=-1)
            return logprobs

        with tracer.invoke(corrupt_prompts):
            logprobs = collect_sperm()
            orig_correct_logprobs = logprobs[range(batch), exp_completions].save()
        
        all_patched_logprobs = {}
        for layer_ix, layer in enumerate(layers):
            for head in range(N_HEADS):
                with tracer.invoke(corrupt_prompts):
                    # Patchi in the value as described
                    z = model.transformer.h[layer].attn.out_proj.input
                    # assert(z.shape == (batch, seq))
                    z_end = z[:, -1, :]
                    z_heads = einops.rearrange(z_end, "batch (h d_head) -> batch h d_head", h=N_HEADS) # How do I get this syntax right?
                    
                    z_heads[:, head] = clean_values[layer][:, head] # Patched it in FOR THE WHOLE BATCH
                    # THen, collect the values
                    patched_logprobs = collect_sperm()
                    # We need the correct token ids
                    # print("Working on ", (layer, head))
                    all_patched_logprobs[(layer, head)] = patched_logprobs[range(batch), exp_completions].save()
                    # print(all_patched_logprobs)
                    

                    # This is shape (b) now, since we've selected 
    # Now some math
    # Computing the difference for each layer, head

    print(all_patched_logprobs)
    for layer_ix, layer in enumerate(layers):
        for head in range(N_HEADS):
            batch_deltas = all_patched_logprobs[(layer, head)] - orig_correct_logprobs
            causal_indirect_effect[layer_ix, head] = batch_deltas.mean() # It should be a scalar now
